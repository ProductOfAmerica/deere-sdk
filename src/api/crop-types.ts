...
export class CropTypesApi {
   ...
   */
  async list(params?: { recordFilter?: string }, options?: RequestOptions): Promise<unknown> {
   ...
   * @generated from GET /cropTypes
   */
  async listAll(params?: { recordFilter?: string }, options?: RequestOptions): Promise<unknown[]> {
    const query = new URLSearchParams();
    if (params?.recordFilter !== undefined) query.set('recordFilter', String(params.recordFilter));
    const queryString = query.toString();
    const path = `/cropTypes${queryString ? `?${queryString}` : ''}`;
   ...
   */
  async get(name: string, options?: RequestOptions): Promise<unknown> {
   ...
   */
  async getCroptypes(id: string, options?: RequestOptions): Promise<unknown> {
   ...
   */
  async listCroptypes(organizationId: string, params?: { recordFilter?: string }, options?: RequestOptions): Promise<unknown> {
    const query = new URLSearchParams();
    if (params?.recordFilter !== undefined) query.set('recordFilter', String(params.recordFilter));
    const queryString = query.toString();
