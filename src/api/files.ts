...
export class FilesApi {
   ...
   */
  async list(params?: { filter: string; fileType1: number; transferable: boolean }, options?: RequestOptions): Promise<unknown> {
   ...
   * @generated from GET /files
   */
  async listAll(params?: { filter: string; fileType1: number; transferable: boolean }, options?: RequestOptions): Promise<unknown[]> {
    const query = new URLSearchParams();
    if (params?.filter !== undefined) query.set('filter', String(params.filter));
    if (params?.fileType1 !== undefined) query.set('fileType1', String(params.fileType1));
    if (params?.transferable !== undefined) query.set('transferable', String(params.transferable));
   ...
   */
  async update(fileId: string, data: Record<string, unknown>, options?: RequestOptions): Promise<void> {
    const path = `/files/${fileId}`;
   ...
   */
  async listFiles(orgId: string, params?: { filter?: string; startDate?: string; endDate?: unknown; fileType1?: number; archived?: boolean; status?: string }, options?: RequestOptions): Promise<unknown> {
    const query = new URLSearchParams();
    if (params?.filter !== undefined) query.set('filter', String(params.filter));
