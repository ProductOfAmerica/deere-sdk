...
export class ClientsApi {
   ...
   */
  async get(orgId: string, clientId: string, params?: { embed?: string }, options?: RequestOptions): Promise<unknown> {
    const query = new URLSearchParams();
    if (params?.embed !== undefined) query.set('embed', String(params.embed));
    const queryString = query.toString();
   ...
   */
  async listFarms(orgId: string, id: string, params?: { name?: string }, options?: RequestOptions): Promise<unknown> {
    const query = new URLSearchParams();
