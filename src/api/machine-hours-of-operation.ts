...
export class MachineHoursOfOperationApi {
   ...
   */
  async list(principalId: string, params?: { startDate?: string; endDate?: string; detailedState?: string }, options?: RequestOptions): Promise<PaginatedResponse<components['schemas']['HoursOfOperation_Response']>> {
   ...
   * @generated from GET /machines/{principalId}/hoursOfOperation
   */
  async listAll(principalId: string, params?: { startDate?: string; endDate?: string; detailedState?: string }, options?: RequestOptions): Promise<components['schemas']['HoursOfOperation_Response'][]> {
    const query = new URLSearchParams();
    if (params?.startDate !== undefined) query.set('startDate', String(params.startDate));
    if (params?.endDate !== undefined) query.set('endDate', String(params.endDate));
    if (params?.detailedState !== undefined) query.set('detailedState', String(params.detailedState));
