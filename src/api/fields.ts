...
export class FieldsApi {
   ...
   */
  async list(orgId: string, params?: { clientName?: string; farmName?: string; fieldName?: string; embed?: string[]; recordFilter?: 'AVAILABLE' | 'ARCHIVED' | 'ALL' }, options?: RequestOptions): Promise<unknown> {
   ...
   * @generated from GET /organizations/{orgId}/fields
   */
  async listAll(orgId: string, params?: { clientName?: string; farmName?: string; fieldName?: string; embed?: string[]; recordFilter?: 'AVAILABLE' | 'ARCHIVED' | 'ALL' }, options?: RequestOptions): Promise<unknown[]> {
    const query = new URLSearchParams();
    if (params?.clientName !== undefined) query.set('clientName', String(params.clientName));
    if (params?.farmName !== undefined) query.set('farmName', String(params.farmName));
    if (params?.fieldName !== undefined) query.set('fieldName', String(params.fieldName));
   ...
   */
  async get(orgId: string, fieldId: string, params?: { embed?: string[] }, options?: RequestOptions): Promise<unknown> {
    const query = new URLSearchParams();
    if (params?.embed !== undefined) query.set('embed', String(params.embed));
    const queryString = query.toString();
   ...
   */
  async update(orgId: string, fieldId: string, data: Record<string, unknown>, options?: RequestOptions): Promise<void> {
   ...
   */
  async listFarms(orgId: string, fieldId: string, params?: { embed?: string[] }, options?: RequestOptions): Promise<unknown> {
    const query = new URLSearchParams();
    if (params?.embed !== undefined) query.set('embed', String(params.embed));
    const queryString = query.toString();
   ...
   */
  async listClients(orgID: string, id: string, options?: RequestOptions): Promise<unknown> {
